...
const Product = () => {
    ...
    {
        let result = await fetch("http://localhost:8000/api/list/"+id);
    ...
    return ( 
        <Container className='rounded pt-2 pb-5 my-4' style={{backgroundColor :"#f2f2f2"}}>
            <Row style={{ width: '75rem', margin: 'auto', marginTop: 100 }}>
                <Col>
                    <Card.Title className='pb-2 text-center'><h2 className='text-primary'>{product && product[0]['name']}</h2></Card.Title>
                    <Card.Img variant="top" src={product && 'http://localhost:8000/'+product[0]['file_path']} />
                </Col>
                <Col className='mx-5 my-5'>
                    <Card.Body className='pb-4'>
                        <Card.Title className='pb-2'><h1>{product && product[0]['name']}</h1></Card.Title>
                        <Card.Text>
                        <h4>{product && product[0]['description']}</h4>
                        </Card.Text>
                    </Card.Body>
                    <ListGroup className="list-group-flush">
                        <Container>
                            ...
                            <Row key={i}>
                              <Col sm={2}>Size : {pro.size}</Col>
                              <Col sm={2}>Rs. {pro.price}</Col>
                            </Row>
                            ))}
                        </Container>
                    </ListGroup>
